...
const SubBarcodeImage = () => {
  ...
  const fetchVariants = async () => {
    try {
      ...
      // Flatten variants (support both "variants" and "productVariants")
      const allVariants = productsArray.flatMap((product) =>
        (product.variants || product.productVariants || []).map((variant) => ({
          ...variant,
      ...
      console.log("Flattened Variants:", allVariants.map(v => v.subBarcode));
      setVariants(allVariants);
    } catch (err) {
      console.error("Failed to fetch products:", err);
    } finally {
