...
const ShopProductAssigner = () => {
  ...
  const [products, setProducts] = useState([]);
  const [selectedShop, setSelectedShop] = useState("");
  const [selectedProduct, setSelectedProduct] = useState(null); // use object for React Select
  const [variants, setVariants] = useState([]);
  const [variantAssignments, setVariantAssignments] = useState([]);
  const [successMessage, setSuccessMessage] = useState("");
    ...
    try {
      do {
        const res = await axiosInstance.get(
          `${process.env.REACT_APP_API_URL}/api/products/pages?page=${page}&limit=100`
        );
  ...
  const handleProductSelect = (product) => {
    ...
    const selectedProductData = products.find((p) => p._id === product.value);
    setVariants(selectedProductData ? selectedProductData.variants : []);
    setVariantAssignments([]);
  };
  ...
  return (
    <div className="p-6 bg-gray-100">
      ...
      <div className="flex flex-col gap-4">
        ...
        {/* Product Selection with React Select */}
        <Select
...
//     try {
//       const res = await axiosInstance.get(
//         `${process.env.REACT_APP_API_URL}/api/products/paginated?page=${pageNumber}&limit=25`
//       );
